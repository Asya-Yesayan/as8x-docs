...
Այսպիսի դասավորության շնորհիվ նույն վալիդատորները կանչվում են ինչպես ինտերֆեյսի (UI) խմբագրման՝ ավտոմատ կերպով, այնպես էլ ծրագրային փոփոխության ժամանակ՝ explicit կերպով կանչելով վալիդացիա։ 
...
## Համակարգային վալիդատորներ

`DocumentHelper.GetStandardGridsValidators(DocumentSchema)`-ը ստեղծում է լռությամբ (default) վալիդատորների հավաքածու՝ ըստ `.as` ֆայլում սահմանված սյան համակարգային տիպի և `IsRequired` հայտանիշի։

| Համակարգային տիպ | `IsRequired = true` | `IsRequired = false` |
|------------------|---------------------|----------------------|
| `StringFieldType` | `StringRequiredValidator` (դատարկ կամ բացատներով արգելված) | `StringValidator` (միայն-բացատների ստուգում) |
| `DateFieldType` | `NonEmptyDateValidator` | `null` |
| `NumericFieldType` | `NonZeroValidator` | `null` |
| `NumericPositiveFieldType` | `NonZeroPositiveValidator` | `null` |
| `BooleanFieldType` | `null` | `null` |

Ստանդարտ պահոցում `GridValidators.CacheValidationResults = true` է, այսինքն վալիդատորի արդյունքը քեշավորվում է մինչև քեշի վերանայումը (ResetForceValidationBit)։

## Custom վալիդատորներ — `IDocumentGridValidationProvider`
...
Ինտերֆեյսն ապահովում է custom վալիդատորների ներդրում (`static abstract`, C# 11)։ Այն իրականացնելու համար `DocumentUI<T>`-ից ժառանգող դասը պետք է իրականցնի IDocumentGridValidationProvider ինտերֆեյսը՝ `GetValidators` մեթոդով, որը կվերադարձնի փաստաթղթի բոլոր աղյուսակների վալիդատորների ամբողջական կոնֆիգուրացիան։

### Կանչման պահը

`DocumentHelper`-ը `DocumentInfo`-ի առաջին ինիցիալիզացիայի ժամանակ (singleton, փաստաթղթի տիպի կտրվածքով) reflection-ով ստուգում է, արդյո՞ք `DocumentUI`-ի տիպը ներդնում է `IDocumentGridValidationProvider`։

* Եթե **այո** — կանչվում է `GetValidators(schema)` մեթոդը։
* Եթե **ոչ** — օգտագործվում է `GetStandardGridsValidators(schema)`-ից վերադարձված հավաքածուն։

Reflection-ի որոնումը ընդգրկում է ինչպես ոչ-հանրային (`NonPublic | Static`), այնպես էլ հանրային (`Public | Static`) ստատիկ ձևերը, ինչը թույլ է տալիս `GetValidators`-ը սահմանել կամ բացահայտ (`ArmSoft.AS8X.Core.UI.Document.IDocumentGridValidationProvider.GetValidators`), կամ չհայտարարված (հանրային ստատիկ) մեթոդով։
...
* սահմանել տողի մակարդակի վալիդատոր (`RowValidator`)՝ սյունային վալիդացիայից հետո,
* անջատել վալիդացիայի քեշավորումը (`CacheValidationResults = false`)։
...
## Վալիդացիայի քեշավորման մեխանիզմ

Քեշը `GridRow`-ի ներքին `PropertyValidatorCache[]` զանգված է՝ ինդեքսավորված `GridColumn.ValidatorCacheIndex`-ով։ Յուրաքանչյուր մուտք պարունակում է երկու դաշտ․

| Դաշտ | Տիպ | Իմաստ |
|------|-----|-------|
| `Message` | `string` | Քեշավորված սխալի հաղորդագրությունը (`null` = վալիդ)։ |
| `ForceValidation` | `bool` | `true` — հաջորդ կանչին վալիդատորը պետք է կրկին կատարվի, քեշը անտեսվում է։ `false` — քեշում առկա արդյունքը վերադարձվում է՝ առանց վալիդատորի կանչի։ |

Քեշը ակտիվ է միայն այն դեպքում, երբ `GridInfo.CacheValidationResults = true`, և սյան `ValidatorCacheIndex >= 0`։ `GridRow.Initialize()`-ի ընթացքում բոլոր մուտքերի `ForceValidation`-ը սահմանվում է `true`, իսկ `Message`-ը՝ `null`։ Այն ժամանակ, երբ վալիդատորը կատարվում է, արդյունքը պահպանվում է քեշում, և `ForceValidation` դառնում է `false`։

### Քեշի վերանայում

| Մեթոդ | Ազդեցություն |
|-------|--------------|
| `ResetForceValidationBit()` | Բոլոր սյուների քեշի `ForceValidation = true`, `Message = null`։ |
| `ResetForceValidationBit(string)` | Միայն տրված ներքին անունով սյան քեշի վերանայում։ |
| `ResetForceValidationBit(string[])` | Տրված սյուների քեշի վերանայում։ |
| `EnforceValidation()` | `ResetForceValidationBit()` + `EnforcePropertyChanged()` — DevExpress-ի requery-ի թողարկում։ |
| `EnforceValidation(string[])` | Տրված սյուների քեշի վերանայում + `EnforcePropertyChanged()`։ |
...
```c#
private string ErrorInfo(string columnName)
{
    if (!this.Grid.NeedsValidation(column)) return null;
    if (column.Validator == null) return null;

    if (this.GridInfo.CacheValidationResults)
    {
        var cached = this.validatorsCache[column.ValidatorCacheIndex];
        if (!cached.ForceValidation)
            return cached.Message;        // քեշից վերադարձ
    }
    string result = column.Validator(this, column);
    if (this.GridInfo.CacheValidationResults)
        CacheValidationResult(column.ValidatorCacheIndex, result);
    return result;
}
```
...
setter-ը կրկին կանչում է `OnPropertyChanged(column.Name)`-ը, բայց **չի վերանայում** քեշը։ Արդյունքում տեղի են ունենում հետևյալ էֆեկտները․
...
```c#
row["Amount"] = 100;
row.ResetForceValidationBit("Amount");      // քեշի վերանայում
...
|-------|-----|----------------|
| [`HasValidationError`](../IGridRow/Properties/HasValidationError.md) | `bool { get; }` | Ընթացիկ տողի վերջին `Validate()`-ի արդյունքը։ |
| [`Validate()`](../IGridRow/Methods/Validate.md) | `string` | Իրականացնում է տողի վալիդացիան, օգտագործելով քեշը, և վերադարձնում առաջին հայտնաբերված սխալը։ |
| [`ColumnHasValidationError(string)`](../IGridRow/Methods/ColumnHasValidationError.md) | `bool` | Տրված սյան վալիդացիայի սխալի առկայության ստուգում (քեշով)։ |
| [`ResetForceValidationBit()`](../IGridRow/Methods/ResetForceValidationBit.md#igridrowresetforcevalidationbit-մեթոդ-1) | `void` | Բոլոր սյուների քեշի վերանայում։ |
| [`ResetForceValidationBit(string)`](../IGridRow/Methods/ResetForceValidationBit.md#igridrowresetforcevalidationbitstring-մեթոդ) | `void` | Տրված սյան քեշի վերանայում։ |
| [`ResetForceValidationBit(string[])`](../IGridRow/Methods/ResetForceValidationBit.md#igridrowresetforcevalidationbitstring-մեթոդ-1) | `void` | Տրված սյուների խմբային վերանայում։ |
| [`EnforceValidation()`](../IGridRow/Methods/EnforceValidation.md#igridrowenforcevalidation-մեթոդ-1) | `void` | `ResetForceValidationBit()` + `EnforcePropertyChanged()`։ |
| [`EnforceValidation(string[])`](../IGridRow/Methods/EnforceValidation.md#igridrowenforcevalidationstring-մեթոդ) | `void` | Տրված սյուների վերանայում + `EnforcePropertyChanged()`։ |
| [`EnforcePropertyChanged()`](../IGridRow/Methods/EnforcePropertyChanged.md) | `void` | կանչում է աղյուսակի առաջին սյան `PropertyChanged` իրադարձությունը (DevExpress requery-ի համար)։ |
| `IDataErrorInfo.this[string]` | `string` | DevExpress XPF-ի կողմից կանչվում է սյան մակարդակում սխալի ստուգման համար։ |
| `IDataErrorInfo.Error` | `string` | DevExpress XPF-ի կողմից կանչվում է տողի մակարդակում սխալի ստուգման համար, փաստացի `Validate()`-ի delegate։ |
...
| Անվանում | Տիպ | Նկարագրություն |
|-------|-----|----------------|
| `RequiresValidation` | `event RequiresValidation` | Իրադարձություն, որով UI-ն կարող է ֆիլտրել վալիդացիան (օրինակ՝ read-only սյուների բացառում)։ |
| `NeedsValidation(GridColumn)` | `bool` | կանչում է `RequiresValidation` իրադարձությունը։ |
| `RowErrorHandler` | `event Action<object, GridRowErrorEventArgs>` | Վալիդատորի մեջ արձակված բացառության notification։ |
| `OnRowError(Exception)` | `void` | կանչում է `RowErrorHandler`-ը (կանչվում է `Validate()` և `ErrorInfo()` մեթոդներից)։ |
...
### `IDocumentGridValidationProvider` (Core.UI)

| Անվանում | Շարահյուսություն | Նկարագրություն |
|-------|----------------|----------------|
| `GetValidators(DocumentSchema)` | `static abstract List<GridValidators>` | Փաստաթղթի բոլոր աղյուսակերի վալիդատորների կոնֆիգուրացիայի վերադարձ։ |
