...
Մեթոդը **հիմնվում է քեշի վրա**․ եթե բոլոր սյուների `forceValidation = false` է, validator-ները կրկին չեն կանչվի, և մեթոդը կցուցադրի նախորդ քեշավորված `Message`-ները։ Հետևաբար, քեշավորված տողի վրա ծրագրային փոփոխությունից հետո `RowValidation`-ից առաջ պետք է կանչել `row.EnforceValidation(columns)`՝ փոփոխված սյուների `forceValidation`-ը `true` սահմանելու համար։
